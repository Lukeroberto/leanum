...
def testVecX : Matrix 3 1 := {
  data := #[1.0, 0.0, 0.0]
  data_size := by rfl
}

def testVecY : Matrix 3 1 := {
  data := #[0.0, 1.0, 0.0]
  data_size := by rfl
}
...
#eval (exp_mat testMatc)[(1,1)]
#eval dot testVecX.T testVecY
#eval inner testVecX.T testVecY
